...
# ══════════════════════════════════════════════════════════════════
    ...
    st.error("🚨 **不要照著這份清單機械式下單。** 用同一個回測引擎、同樣成本、"
             "同樣出場規則測過：把這個型態當進場訊號跑 10 年是 **-50.2%**、勝率僅 12.8%"
             "（同期 AI 選股 +328%）。原因是**進場型態與出場規則不匹配**——突破後回測箱頂"
             "是常態，現行的固定 8% 停損會一再被打到。**已實測**把停損改成守突破K棒低點，"
             "可拉到 **-9.1%**、賺賠比 1.44→3.55，但**仍是負的**。這份清單的用途是"
             "**練手感、練自己的出場**，不是一份可以照抄的買單。")

    with st.expander("這個型態的 10 年驗證數據（誠實版）", expanded=False):
        st.markdown(
            "**事件研究（買進後抱N天、無停損的平均超額報酬）**\n"
            "- 型態池本身：CAR20 **+0.64%**、CAR60 **+1.18%**\n"
            "- 型態池 → AI 因子排序前20：CAR20 **+1.17%**、CAR60 **+2.18%**（排序有鑑別力）\n"
            "- 與 AI 主軌只有 **14.5%** 重疊（40天裡17天完全不重疊）→ 互補的另一個維度\n"
            "- 但 CAR20 的 1.17% 只略高於來回摩擦成本約 1.07%，平均持有 15.5 天正落在邊緣；"
            "CAR40/60 才明顯覆蓋。\n\n"
            "**組合回測（套用現行停損/停利/部位控管/成本）**\n"
            "- 型態當進場濾網：總報酬 **-50.2%**、Sharpe **-0.85**、回撤 -50.7%、"
            "勝率 **12.8%**、僅 78 筆\n"
            "- 同期 AI 選股：+328.0%、Sharpe 0.97、回撤 -27.4%、勝率 34.2%、483 筆\n"
            "- ⚠️ **但同期 0050 買進持有是 +745.5%（Sharpe 1.08）**——AI 軌雖然遠勝練習軌，"
            "拉到 11.5 年全期其實是**輸給大盤**的，只贏在回撤較小。詳見首頁「回測誠實聲明」。\n\n"
            "**兩者為何差這麼多**：CAR 是「無停損抱N天」，組合回測套了 8% 固定停損。"
            "現行出場是為「騎乘已確立的趨勢」設計的，而突破是趨勢剛啟動、必然伴隨回測，"
            "兩者波動特性不同 → 停損一再被觸發。**這說明的是出場規則不匹配，"
            "不是型態本身無效**（事件研究顯示型態後仍有正向漂移）。\n\n"
            "**出場配對實驗（2026-07-24，5 個變體）**——驗證上述推論：\n"
            "| 停損方式 | 總報酬 | 勝率 | 賺賠比 | 筆數 |\n"
            "|---|---|---|---|---|\n"
            "| 固定 8%（現況） | -50.2% | 12.8% | 1.44 | 78 |\n"
            "| **守突破K棒低點** | **-9.1%** | 20.6% | **3.55** | 189 |\n"
            "| 守箱型低點 | -61.3% | 17.1% | 0.87 | 70 |\n"
            "| 箱型低點+移停降20% | -61.0% | 21.1% | 1.03 | 90 |\n"
            "| 箱型低點+移停降15% | -59.5% | 23.9% | 0.79 | 88 |\n\n"
            "結論：**推論被證實但幅度不足**。守突破K棒低點把賺賠比推到接近 AI 主軌"
            "（3.55 vs 4.79），總報酬改善 41 個百分點，但終究還是負的；守箱型低點更差"
            "（停損位太遠 → 單筆虧損放大，賺賠比掉到 1 以下）。依「A/B 沒有明確勝出就"
            "不改預設」的規矩，`stop_mode` 保留為 opt-in、**預設關閉**。\n\n"
