...
## 漲跌停鎖死判定：從 blocker 改為凍結規則（2026-08-12 晚）

F0 的 remaining blocker「official TWSE locked-limit state is not present in the
named release」實際上**不是下載問題**：官方日行情根本沒有「是否鎖死」這個欄位。
本輪把它改成一條在查看任何績效前凍結的判定規則，寫入 SPEC §7.5.1。

### 先否決了直覺解法

最直覺的做法是「`change_pct >= 6.9` 即漲停」。實測 1,917,768 筆價格後否決：
...
**分布平滑、沒有斷點。** 原因是漲停價必須落在合法檔位，跨級距時會明顯低於 7%
——參考價 9.99 元的漲停價落在 0.05 檔位區間而成為 10.65 元，僅 +6.61%。
以 6.9% 為門檻會漏掉 **54%** 的真實漲停日。任何門檻都是啟發式，
違反「不得發明代理指標」。

### 採用的規則

1. 由官方 `change_pct` 反推當日參考價：`close / (1 + change_pct/100)`。
   交易所本就以當日參考價為分母，因此不必自行尋找前一個收盤價，
   同時避開 D3 記錄的 182 筆參考價重設。
2. 以官方升降單位計算漲跌停價（2005~2014 為 ±7%），檔位由候選價本身的級距決定。
3. 判定鎖死須**同時**滿足：收盤價精確等於漲跌停價，且當日
   `open == high == low == close`。
4. 漲跌停價與參考價相差不足一個檔位時（極低價股）概念退化，判為非鎖死。

第 3 條的第二個條件是關鍵：若當日曾在漲停價以外成交，代表有對手盤，
開盤市價單本來就會成交；只有整日單一價才代表排隊未消化。

### 實測

限 `close >= 10`（MOM-1 §7.1.4 門檻）的 1,545,472 筆：

- 漲停日 39,760 筆，`change_pct` 6.496~7.000，下界與跨級距進位理論值一致；
  其中整日單一價者 3,140 筆（7.9%）。
- 跌停日 17,392 筆，整日單一價者 1,245 筆（7.2%）。
- 接入 F0 後，1,070 個有 T+1 開盤價的股-月中有 **6 個**因漲停鎖死被擋，
  可 sizing 者由 1,070 降為 1,064。被擋的訂單維持未成交，不以收盤價或
  前值頂替。
...
| 診斷 | 前值 | 現值 |
|---|---|---|
| release 訊號診斷 | `29CB56DA…` | **`50DED922…`** |
| F0 執行稽核 | `95751DC4…` | **`BF815DBF…`** |

兩個 verifier 均 `passed: true`，`f0_status` 維持 `blocked`。

### F0 阻擋項：3 → 2

僅剩「TWSE 停止交易無獨立官方旗標」與「D3 實際股數 ledger 未對帳」。

2026-08-12 本輪測試（`.venv-repro`，排除兩個需 DB 連線的模組）：
676 passed、0 failed、4 個既有 warnings。

## F0 通過（2026-08-13）

兩個 remaining blocker 全部關閉，`f0_status` 由 `blocked` 改為 `passed`。

### 停止交易旗標：實測為零影響

官方 `TWTAWU`（集中市場暫停交易證券）**只回溯到民國 100 年 10 月 3 日**
（2011-10-03），更早直接回「查詢日期小於100年10月3日」。且該端點預設忽略
`date` 參數（同 TWT48U 的陷阱），必須用 `startDate`/`endDate` 才查得到歷史。

可查窗口（2011-10~2014）共 28 筆，**沒有任何一檔是四位數本地普通股**：
存託憑證 13、普通股 11（全為 91xxxx 外國企業來台第一上市）、權證類 4。
11 筆普通股中 9 筆停牌當日無行情列（已被 `price_present` 排除），
另 2 筆有行情但收盤價分別為 3.67 與 1.18，低於 §7.1.4 的 10 元門檻。

**與 MOM-1 曾選中的 239 檔相異證券交集為空集合。**

殘餘風險據實記錄：2005~2011-09 無法驗證。支撐外推的是結構性論證
（停牌→無行情→被排除，與年份無關），是有依據的推論而非證明。

### D3 實際股數 ledger：凍結兩條政策（SPEC §7.5.2）

653 筆 blocked 中**只有 10 筆**落在 MOM-1 實際持有期間內
（1,070 個股-月中的 10 個，0.93%），另有 56 筆 executable 落在持有期間內。

| 阻擋原因 | 筆數 | 政策 |
|---|---:|---|
| 現金增資條件／交割時點缺失 | 9 | A：一律不認購 |
| 官方無償配股條件缺失 | 1 | B：事件前收盤價平倉 |

政策 A 之所以成立，是因為現金增資**可以選擇不認購**：股數不變、不付款、
價格吃除權跌幅，完全不需要那些缺失的條件，且方向保守（低估報酬）。
政策 B 處理的無償配股是強制的、股數必變，此時不猜倍率而是結清轉現金，
影響 0.09% 的股-月。

明文拒絕從價格反推倍率：除權參考價變動同時含員工紅利與現增稀釋，
那些不是舊股東拿到的股票；D3 第三輪已因此把 50 筆價格反推事件降回 blocked。

### §9.1 逐條覆蓋

診斷 JSON 的 `spec_9_1_coverage` 記錄六項要求對應的測試檔。
`f0_status_meaning` 明確界定：這只主張實作正確性，不揭露任何 holdout 績效，
且依 §9.2 的重新定位，backward holdout 是否證測試，永遠無法證明策略有效。

### 診斷雜湊

| 診斷 | 現值 |
|---|---|
| release 訊號診斷 | `10ADCED3…` |
| F0 執行稽核 | `0A46959E…` |

兩個 verifier 均 `passed: true`。

**F1 現在具備開封條件**，尚待：變體與門檻維持不變、且開封指令先提交。

2026-08-13 測試：708 passed、0 failed、6 warnings（含 2 個新增測試檔帶入的
既有 pandas resample deprecation）。
