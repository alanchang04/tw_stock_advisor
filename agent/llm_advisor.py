...
5. 若最終 5 檔入選名單裡有任何一檔不在你自己列出的 backups 名單中（例如：某檔
   因空方 VETO 被你剔除後，你直接從候選池另外挑了一檔遞補，而不是用 backups
   遞補），必須在該檔的 reason 裡明確說明為什麼 backups 不夠好、為何這檔候選更
   值得選——不能沒有交代就跳過自己列的 backups，那代表你自己的判斷前後不一致。
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if result.get("recommendations"):
                    ...
                    _bypassed = [r["stock_id"] for r in result["recommendations"] if r.get("bypassed_backups")]
                    rec.summary = (f"裁決選出 {len(picks)} 檔：{', '.join(picks)}"
                                   ...
                                   + (f"（🧨 跳過自己的backups {len(_bypassed)} 檔："
                                      f"{', '.join(_bypassed)}）" if _bypassed else ""))
...
def format_report(result: dict) -> str:
    ...
    for rec in result["recommendations"]:
        ...
        if rec.get("bypassed_backups"):
            lines.append("   🧨 此檔未經多方主張，且裁決跳過自己列的backups直接從候選資料另挑（判斷前後不一致，多留意）")
        elif rec.get("not_debated"):
