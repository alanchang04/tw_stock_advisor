...
STRATEGY = {
    ...
    # ── 研究宇宙市場別限制（2026-08-06，EXPERIMENTS.md「資料覆蓋偏誤」）──
    # None = 不限制（上市+上櫃），**正式/實盤維持 None**——實盤資料是完整的，
    # 沒有理由放棄櫃買機會。
    #
    # 研究則需要 ("TWSE",)：櫃買法人資料 2018-01 才開始，而 development 是
    # 2015~2020，等於前三年櫃買股拿不到權重 6.0 的法人因子（fillna(0)），
    # 選股實際退化成只選上市；2018 起櫃買股才進得了候選池。**同一個 development
    # 區間裡宇宙換了一次**，任何跨年度比較都不成立。
    #
    # 限制成上市股是「一致的窄定義」勝過「不一致的寬定義」，屬寫實度/方法論修正，
    # 與 exclude_disposition 同類：**採用與否不取決於報酬高低**。
...
#  策略版本沿革（2026-07-23）
#
#  ⚠️ 改完 STRATEGY 並**推上 GitHub** 後，要在這裡加一筆。
#
#  為什麼要有這張表：🔄歷史績效頁原本寫死一個 `STRATEGY_V2_DATE` 常數，
#  當初為 2026-07-04 那次出場規則改版設成 07-05，之後策略又大改了好幾輪
#  （P1因子重配權、出場規則消融、族群曝險上限、熊市擋新倉）卻沒人回頭更新它，
#  結果頁面長期把「舊策略的交易」標成「現行策略績效」。寫死常數必然會再犯，
#  改成清單後，新增版本＝加一筆，頁面自動多一個分段。
#
#  **live_from 是「該版程式碼真正在排程上跑出來的部位進場日」，不是 commit 日**。
#  這兩者可能差很多：2026-07-20~22 的那批改動直到 07-22 晚上才 push，
#  在那之前 GitHub Actions 每晚抓的都是舊碼（查 execution_log 的 judge payload
#  有沒有新版才有的欄位可以驗證是哪一版在跑）。訊號當晚產生、隔日開盤成交，
#  所以 live_from = 第一次跑新碼的隔一個交易日。
# ══════════════════════════════════════════════════════════════════
...
STRATEGY_ERAS = [
    {
        "key": "v3",
        "label": "現行策略",
        "live_from": _date(2026, 7, 24),
        "desc": "P1因子IC重配權（rs20/動能歸零、營收升權）＋出場規則消融（牛市關死叉）"
                "＋族群曝險上限0.6＋空頭濾網擋新倉。10年回測 +344%/Sharpe1.00/回撤-24.8%。",
    },
    {
        "key": "v2",
        "label": "舊策略（出場改版，但無熊市防護）",
        "live_from": _date(2026, 7, 5),
        "desc": "關均線出場＋動能/營收因子。此版沒有空頭濾網，空頭時仍持續開新倉，"
                "實盤上就是「一直買然後停損殺出」的那段。",
    },
    {
        "key": "v1",
        "label": "最早期版本",
        "live_from": _date(2000, 1, 1),      # 下界，代表「更早的全部」
        "desc": "單日技術面事件旗標主導選股的初版。",
    },
]


def strategy_era_for(entry_date) -> dict:
    """某筆交易的進場日屬於哪個策略版本（回傳 STRATEGY_ERAS 的其中一筆）。"""
    for era in STRATEGY_ERAS:                 # 已由新到舊排列
        if entry_date >= era["live_from"]:
            return era
    return STRATEGY_ERAS[-1]
