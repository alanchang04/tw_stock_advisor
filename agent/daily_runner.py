...
def run_daily_recommendation():
    logger.info("=" * 50)
    logger.info(f"每日流程開始：{date.today()}")
    logger.info("=" * 50)

    eval_date = _latest_trade_date() or date.today()

    # Step 1: 更新族群輪動熱度
    logger.info("Step 1/5 — 計算族群輪動熱度")
    run_sector_momentum()

    # Step 2: 出場檢查（先做，且不依賴 LLM —— 確保賣出提醒一定會發）
    logger.info("Step 2/5 — 檢查持有部位出場訊號")
    exits = evaluate_exits(eval_date)

    # Step 3: 取熱門產業 + 篩選候選股票
    logger.info("Step 3/5 — 篩選候選股票")
    ...
    if candidates is not None and not candidates.empty:
        # Step 4: 呼叫 LLM 產生推薦
        logger.info("Step 4/5 — 呼叫 LLM 產生推薦")
    ...
    # Step 5: 合併報告（進場推薦 + 出場提醒）
    logger.info("Step 5/5 — 產出報告")
    report = format_report(result) if result else "📊 今日無新進場推薦"
