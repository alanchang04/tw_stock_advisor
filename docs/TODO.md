...
- ✅ P1 全部完成（2026-07-05）：P1-1 防重複索引（migration 11 已跑）、P1-2 時區 tw_today()、P1-3 YT 無字幕標題推測降級、P1-4 單元測試 27 個全綠（tests/）、P1-5 etf_fetcher 不再重放 migration。
- ✅ 額外：新聞深度摘要改接**鉅亨網 API**（有全文，Google News 只有標題）→ 每篇 3~5 句條列重點；YT 摘要升級 4~6 點條列；歷史績效頁加「新策略/全部歷史」過濾（新策略 = 2026-07-05 後進場）。
- ⬜ 待做：P2 全部。策略凍結中（觀察期至 ~2026-08-31），僅修 bug 不調參。市況轉空時優先回測開回 exit_on_death_cross（見 ablation_result.md）。
