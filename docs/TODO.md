...
- ✅ 已完成：P0-1（回測交易成本）、P0-3（彙整去雜訊）、P0-4（workflow 依賴收斂，FinMind 依使用者要求保留）、**Epic A（我的持倉）**、**Epic B（追蹤清單）**、**Epic C（族群輪動）**。Migration 07~09 已直接在 Neon 執行完畢。
- ⬜ 待做：P0-2（出場規則消融測試——現況每筆淨報酬 +2.04% 但勝率僅 32%，急需找出過度出場的規則）、P1-1~P1-5、P2 全部。
